		 ...
		 int offset, int verbose)
{
    ...
    if (pic->flags & PIC_FLAG_TOP_FIELD_FIRST)
        fprintf(f, "\nframe %u is top field first\n", pic->temporal_reference);
    if (pic->flags & PIC_FLAG_PROGRESSIVE_FRAME)
        fprintf(f, "frame %u is progressive frame\n", pic->temporal_reference);
    if (pic->flags & PIC_FLAG_REPEAT_FIRST_FIELD)
        fprintf(f, "frame %u is repeat first field\n", pic->temporal_reference);

    fprintf(f, "FLAGS                           FLAGS FLAGS %u\n", pic->flags);
    fprintf(f, "frame period is equal to %u\n\n", 27000000 / seq->frame_period);
